...
{ 
			...
			for (int zz = z - r; zz <= z + r; zz++)
			{
				...
				if (d <= r - 0.5)
				{
					if (yy < y)
					{
						...
						{
							//level->setTile(xx, yy, zz, Tile::bedrock_Id);
							placeBlock(level, xx, yy, zz, Tile::bedrock_Id, 0);
						}
					}
					else if (yy > y)
					{
	...
	placeBlock(level,x, y + 0, z,	 Tile::bedrock_Id, 0);
	...
	placeBlock(level,x, y + 2, z + 1, Tile::torch_Id, 0);
	placeBlock(level,x, y + 3, z,	 Tile::bedrock_Id, 0);
	//placeBlock(level,x, y + 4, z,	 Tile::dragon_egg_Id, 0);
	...
	for (int yy = y - 5; yy < y - 1; yy++)
	{
		for (int xx = x - (r - 1); xx <= x + (r - 1); xx++)
		{
			for (int zz = z - (r - 1); zz <= z + (r - 1); zz++)
			{
				...
				{
					placeBlock(level, xx, yy, zz, Tile::end_stone_Id, 0);
				}
			}
		}
	}
