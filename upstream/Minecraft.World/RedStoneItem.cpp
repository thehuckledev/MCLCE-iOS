...
bool RedStoneItem::useOn(shared_ptr<ItemInstance> itemInstance, shared_ptr<Player> player, Level *level, int x, int y, int z, int face, float clickX, float clickY, float clickZ, bool bTestUseOnOnly) 
{
	...
	if (level->getTile(x, y, z) != Tile::snow_layer_Id)
	{
		if (face == 0) y--;
		if (face == 1) y++;
		if (face == 2) z--;
	...
	if (Tile::redStoneDust->mayPlace(level, x, y, z))
	{
		...
		{	
			...
			player->awardStat(GenericStats::blocksPlaced(Tile::redstone_wire_Id), GenericStats::param_blocksPlaced(Tile::redstone_wire_Id,itemInstance->getAuxValue(),1));

			itemInstance->count--;
			level->setTileAndUpdate(x, y, z, Tile::redstone_wire_Id);
		}
	}
