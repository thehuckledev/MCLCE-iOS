...
bool DesertWellFeature::place(Level *level, Random *random, int x, int y, int z)
{
	...
	// place water cross
	level->setTileAndData(x, y, z, Tile::flowing_water_Id, 0, Tile::UPDATE_CLIENTS);
	level->setTileAndData(x - 1, y, z, Tile::flowing_water_Id, 0, Tile::UPDATE_CLIENTS);
	level->setTileAndData(x + 1, y, z, Tile::flowing_water_Id, 0, Tile::UPDATE_CLIENTS);
	level->setTileAndData(x, y, z - 1, Tile::flowing_water_Id, 0, Tile::UPDATE_CLIENTS);
	level->setTileAndData(x, y, z + 1, Tile::flowing_water_Id, 0, Tile::UPDATE_CLIENTS);
	...
	level->setTileAndData(x + 2, y + 1, z, Tile::stone_slab_Id, StoneSlabTile::SAND_SLAB, Tile::UPDATE_CLIENTS);
	level->setTileAndData(x - 2, y + 1, z, Tile::stone_slab_Id, StoneSlabTile::SAND_SLAB, Tile::UPDATE_CLIENTS);
	level->setTileAndData(x, y + 1, z + 2, Tile::stone_slab_Id, StoneSlabTile::SAND_SLAB, Tile::UPDATE_CLIENTS);
	level->setTileAndData(x, y + 1, z - 2, Tile::stone_slab_Id, StoneSlabTile::SAND_SLAB, Tile::UPDATE_CLIENTS);
	...
	for (int ox = -1; ox <= 1; ox++)
	{
		for (int oz = -1; oz <= 1; oz++)
		{
			...
			{
				level->setTileAndData(x + ox, y + 4, z + oz, Tile::stone_slab_Id, StoneSlabTile::SAND_SLAB, Tile::UPDATE_CLIENTS);
			}
		}
	}
